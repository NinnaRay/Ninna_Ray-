...
import { storage } from "../../storage"; // Import user storage
...
export function registerChatRoutes(app: Express): void {
  ...
  app.get("/api/conversations", async (req: Request, res: Response) => {
    try {
      ...
      let conversations;
      if (userId) {
        conversations = await chatStorage.getConversationsByUser(userId);
      } else {
...
Keep responses concise and engaging for a chat interface.`;
      ...
      const stream = await openai.chat.completions.create({
        ...
        stream: true,
        max_completion_tokens: 500, // Keep it relatively short
      });
